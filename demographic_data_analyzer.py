...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(
        (df[df.education == "Bachelors"].education.count() / count_people) * 100, 1
    )
    ...
    rich_percentage = round(
        (
            ...
            / df[df["hours-per-week"] == min_work_hours]["salary"].count()
        )
        * 100,
        1,
    )
    ...
    all_earners = (
        ...
        .count()
        .join(high_earners, on="native-country")
    )
    highest_earning_country_series = (
        (all_earners.total_count / all_earners.salary)
        .sort_values(ascending=False)
        .head(1)
    )
    highest_earning_country = highest_earning_country_series.index.item()
    highest_earning_country_percentage = round(
        highest_earning_country_series.item() * 100, 1
    )

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation_series = (
